...
//
// The old app used jQuery to do a lot of the book-keeping that the browser
// already knows how to do. This module replaces those helpers with plain DOM
// and canvas code so the landing page has no dependency on jQuery.
...
export function resizeButton(svgButton, innerCircle, outerCircle, viewportHeight) {
    ...
    const radius = Math.floor(viewportHeight / 14);
...
class Sentence {
    constructor(canvas, endpoint = "/sentence.php") {
        this.sentence = null;
        this.author = null;
        this.canvasWidth = canvas.width;
        this.canvasHeight = canvas.height;
    ...
    draw(deltaT) {
        ...
        // Fade logic is controlled by the subclass, but drawing is shared.
    ...
    async requestSentence() {
        ...
        try {
            // The seed keeps the old deterministic selection behavior, but the
            // browser now sends it with plain fetch instead of jQuery.ajax.
...
export class FallingSentence extends Sentence {
    ...
    createNew() {
        // Longer sentences are drawn smaller so they still fit on screen.
